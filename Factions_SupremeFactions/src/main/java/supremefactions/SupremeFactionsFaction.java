...
/**
 * SupremeFactions implementation of {@link cc.javajobs.factionsbridge.bridge.infrastructure.struct.Faction}.
...
public class SupremeFactionsFaction extends AbstractFaction<Faction> {
    ...
    public void setBank(double balance) {
        faction.takeMoneyFromFBalance(getBank());
        faction.depositMoneyToFBalance(balance);
    }
    ...
    public Location getWarp(@NotNull String name) {
        final LazyLocation location = faction.getWarp(name);
    ...
    public void clearStrikes() {
        final ArrayList<Strike> strikes = new ArrayList<>(faction.getStrikes());
        for (Strike strike : strikes) faction.removeStrike(strike);
    }
    ...
    public void addStrike(String sender, String reason) {
        faction.addStrike(new Strike(reason, sender, System.currentTimeMillis()));
    }
    ...
    public void removeStrike(String sender, String reason) {
        final ArrayList<Strike> strikes = new ArrayList<>(faction.getStrikes());
        Strike chosenStrike = strikes.stream()
                .filter(strike -> strike.getStriker().equals(sender))
                .filter(strike -> strike.getReason().equals(reason))
                .findFirst()
                .orElse(null);
        if (chosenStrike == null) {
            if (bridge.catch_exceptions) return;
            methodError(getClass(), "removeStrike(Sender, String)", "RemoveMe == null");
            return;
        }
        faction.removeStrike(chosenStrike);
    }
