...
/**
 * KoreFactions implementation of {@link cc.javajobs.factionsbridge.bridge.infrastructure.struct.Faction}.
 * Object Target: {@link Faction}.
 */
public class KoreFactionsFaction extends AbstractFaction<Faction> {
     ...
     */
    public KoreFactionsFaction(@NotNull Faction faction) {
        super(faction);
    }
    ...
    public FPlayer getLeader() {
        return new KoreFactionsFPlayer(faction.getFPlayerAdmin());
    }
    ...
    public List<Claim> getAllClaims() {
        return faction.getAllClaims().stream().map(KoreFactionsClaim::new).collect(Collectors.toList());
    }
    ...
    public List<FPlayer> getMembers() {
        return faction.getFPlayers().stream().map(KoreFactionsFPlayer::new).collect(Collectors.toList());
    }
    ...
    public void clearStrikes() {
        for (int i = 0; i < faction.getStrikes().size(); i++) faction.takeStrike(i);
    }
    ...
    public void addStrike(String sender, String reason) {
        faction.addStrike(reason, System.currentTimeMillis());
    }
    ...
    public void removeStrike(String sender, String reason) {
        int target = -1;
        final List<StrikeInfo> strikes = faction.getStrikes();
        for (int i = 0; i < strikes.size(); i++) {
            if (strikes.get(i).getStrikeReason().equals(reason)) {
                target = i;
                break;
            }
        }
        if (target < 0) {
            if (!bridge.catch_exceptions) methodError(
                    getClass(), "removeStrike(sender, reason)", "Failed to find Strike from Reason."
            );
            return;
        }
        if (!faction.takeStrike(target)) {
            if (!bridge.catch_exceptions) methodError(
                    getClass(), "removeStrike(sender, reason)", "Failed to take the Strike."
            );
        }
    }
